...
const buildAttentionItems = (summary: NotificationSummary | null): AttentionItem[] => {
  if (!summary) return []
  ...
  const items: Array<AttentionItem | null> = [
    ...
    summary.lowStockVariants > 0 ? {
      ...
      detail: 'Biến thể còn không quá 5 sản phẩm khả dụng.',
  ...
  return items.filter((item): item is AttentionItem => item !== null).slice(0, 6)
}
        ...
        <div className="admin-dash-attention-list">
          {items.map((item) => (
            <button type="button" key={item.key} onClick={() => navigateAdmin(item.href)}>
              <i className={`is-${item.tone}`} aria-hidden="true">{item.icon}</i>
              <span>
