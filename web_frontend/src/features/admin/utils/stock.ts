...
export const isLowStockQuantity = (availableQuantity: number) =>
  availableQuantity > 0 && availableQuantity <= lowStockThreshold

export const getStockMeta = (items: StockQuantity[]) => ({
  total: items.reduce((sum, item) => sum + item.availableQuantity, 0),
  low: items.filter((item) => isLowStockQuantity(item.availableQuantity)).length,
  out: items.filter((item) => item.availableQuantity === 0).length,
})

export const getStockStatus = (items: StockQuantity[]) => {
  if (items.length === 0 || items.every((item) => item.availableQuantity === 0)) {
    return { id: 'out' as const, label: 'Hết hàng', className: 'is-out' }
  }

  if (items.some((item) => isLowStockQuantity(item.availableQuantity))) {
