...
export function MyReviewsPage() {
  ...
  const load = async () => {
    const [eligibleResult, reviewResult] = await Promise.all([
      reviewService.listEligibleItems(),
      reviewService.listMyReviews(),
    ])
    setEligible(eligibleResult.items.filter((item) => item.canReview))
    setReviews(reviewResult.items)
  }

  useEffect(() => {
    void load()
      .catch((error) => message.error(error instanceof Error ? error.message : 'Không thể tải đánh giá'))
  ...
  const remove = async (id: string) => {
    ...
    try {
      ...
      message.error(error instanceof Error ? error.message : 'Không thể xóa đánh giá.')
    }
  }
  ...
  const saveEdit = async (input: EditFormState) => {
    ...
    if (input.comment.trim().length < 10) {
      message.warning('Nội dung cần ít nhất 10 ký tự.')
      return
    }
  ...
  return (
    <MainLayout>
                ...
                {eligible.length ? eligible.map((item) => (
                  <article className="my-review-card my-review-card--eligible" key={item.orderItemId}>
                    ...
                    <Button
                      ...
                      href={`/products/${item.product._id}?compose=1&orderId=${item.orderId}&orderItemId=${item.orderItemId}`}
                    >
                      Viết đánh giá
                    </Button>
                  </article>
      ...
      <EditReviewModal review={editingReview} saving={saving} onCancel={() => setEditingReview(null)} onSave={saveEdit} />
    </MainLayout>
  )
}
...
export function EditReviewModal({ review, saving, onCancel, onSave }: {
  ...
  const onPickImages = (files: File[]) => {
    ...
    if (oversized) {
      message.warning(`${oversized.name} vượt quá 5MB.`)
      return
    }

    const remaining = 5 - form.keepImageIds.length - form.newImages.length
    if (remaining <= 0) {
      message.warning('Đã đạt giới hạn 5 ảnh mỗi đánh giá.')
      return
    }

    if (files.length > remaining) {
      message.warning(`Chỉ còn có thể thêm ${remaining} ảnh.`)
      return
    }
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 4 }}>
              ...
              {review.images.map((image) => (
                <span
                  ...
                  style={{ position: 'relative', opacity: image._id && form.keepImageIds.includes(image._id) ? 1 : 0.35, cursor: 'pointer' }}
                  onClick={() => image._id && toggleKeep(image._id)}
                >
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {imagePreviews.map(({ file, url }) => (
              <span key={`${file.name}-${file.lastModified}`} style={{ position: 'relative' }}>
                ...
                <button
                  type="button"
                  onClick={() => setForm((current) => current ? { ...current, newImages: current.newImages.filter((item) => item !== file) } : current)}
                  style={{ position: 'absolute', top: -6, right: -6, background: 'rgba(0,0,0,.6)', color: '#fff', border: 'none', borderRadius: '50%', width: 18, height: 18, cursor: 'pointer' }}
                >
                  x
                </button>
              </span>
