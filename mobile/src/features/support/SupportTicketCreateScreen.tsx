...
const categories: Array<[SupportCategory, string]> = [['orders', 'Đơn hàng'], ['shipping', 'Giao hàng'], ['returns', 'Đổi trả'], ['payments', 'Thanh toán'], ['promotions', 'Voucher'], ['loyalty', 'Thành viên'], ['account', 'Tài khoản'], ['app_website', 'Ứng dụng'], ['other', 'Khác']];
...
export default function SupportTicketCreateScreen() {
  ...
  const chooseImages = async () => {
    ...
    if (!result.canceled) setImages(result.assets.slice(0, 3).map((asset, index) => ({ uri: asset.uri, name: asset.fileName || `support-${Date.now()}-${index}.jpg`, type: asset.mimeType || 'image/jpeg' })));
  };
  ...
  const submit = async () => {
    ...
    try {
      const result = await runWithAuth((token) => supportApi.createTicket(token, { type, category, subject: subject.trim(), body: body.trim(), requiresReply, orderId: orderId.trim() || undefined, couponCode: couponCode.trim() || undefined, images }));
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'Không thể gửi yêu cầu.'); } finally { setLoading(false); }
  };
